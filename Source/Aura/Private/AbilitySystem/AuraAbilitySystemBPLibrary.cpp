...
bool UAuraAbilitySystemBPLibrary::MakeWidgetControllerParam(const UObject* WorldContextObject, FWidgetControllerParams& OutWidgetControllerParams, AMyHUD*& OutMyHUD)
{
...
UOverlayWidgetController* UAuraAbilitySystemBPLibrary::GetOverlayWidgetController(const UObject* WorldContextObject)
{
	...
	if (MakeWidgetControllerParam(WorldContextObject,WCParams,MyHUD))
	{
...
{
	const AMyGameMode* AuraGameMode = Cast<AMyGameMode>(UGameplayStatics::GetGameMode(WorldContextObject));
	if (AuraGameMode == nullptr) return nullptr;
	...
	bool IsSuccessfulDebuff)
{
	if ( FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		return AuraGameplayEffectContext->SetSuccessfulDebuff(IsSuccessfulDebuff);
	}
}

void UAuraAbilitySystemBPLibrary::SetDamageDebuff(FGameplayEffectContextHandle& EffectContextHandle,
	float InDamageDebuff)
{
	if ( FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		return AuraGameplayEffectContext->SetDebuffDamage(InDamageDebuff);
	}
}
	...
	float InDebuffDuration)
{
	if ( FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		return AuraGameplayEffectContext->SetDebuffDuration(InDebuffDuration);
	}
}
	...
	float InDebuffFrequency)
{
	if ( FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		return AuraGameplayEffectContext->SetDebuffFrequency(InDebuffFrequency);
	}
}
	...
	const FGameplayTag& InDamageType)
{
	if ( FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		...
		return AuraGameplayEffectContext->SetDamageType(DamageType);
	}
}
	...
	const FVector& InImpulse)
{
	if ( FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		return AuraGameplayEffectContext->SetDeathImpulse(InImpulse);
	}
}
...
void UAuraAbilitySystemBPLibrary::GetClosestTargets(int32 MaxTarget, const TArray<AActor*>& Actors,
...
TArray<FRotator> UAuraAbilitySystemBPLibrary::EvenlySpacedRotators(const FVector& Forward, const FVector& Axis,float Spread, int32 NumRotators)
{
	...
	const FVector LefOfSpread = Forward.RotateAngleAxis(- Spread /2.f, FVector::UpVector);
	...
	if (NumRotators > 1)
	{
		...
		{
			const FVector Direction = LefOfSpread.RotateAngleAxis(DeltaSpread * i, FVector::UpVector);
...
TArray<FVector> UAuraAbilitySystemBPLibrary::EvenlyRotatedVectors(const FVector& Forward, const FVector& Axis,float Spread, int32 NumVectors)
{
	...
	const FVector LefOfSpread = Forward.RotateAngleAxis(-Spread /2.f, Axis);
	if (NumVectors > 1)
	{
